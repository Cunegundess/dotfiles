...
if [[ $# -eq 1 ]]; then
    ...
    selected=$(
        find "$HOME" -mindepth 1 -maxdepth 3 -type d | fzf --border --print-query --preview 'ls -la {}'
    )
...
if [[ ! -d $selected ]]; then
    ...
    if [[ $selected == https://github.com/* || $selected == git@github.com:* || $selected == https://gitlab.com/* || $selected == git@gitlab.com:* ]]; then
        repo_name=$(basename "$selected" .git)
        echo "Cloning repo '$repo_name' to '${DEV_DIR}/$repo_name'."
