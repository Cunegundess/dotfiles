require("gitsigns").setup({
	...
	on_attach = function(bufnr)
		...
		end, { desc = "Jump to next git [h]unk", buffer = bufnr })
		...
		end, { desc = "Jump to previous git [h]unk", buffer = bufnr })
		...
		vim.keymap.set("n", "<leader>hu", gitsigns.undo_stage_hunk, { desc = "git [u]ndo stage hunk", buffer = bufnr })
