...
# Configuration
WALLPAPER_DIR="$HOME/dotfiles/wallpapers/wallpapers/" # Change this to your wallpaper directory
...
#     "$SHUFFLE_ICON"
magick -size "${THUMBNAIL_WIDTH}x${THUMBNAIL_HEIGHT}" xc:#1e1e2e \
    \( "$HOME/dotfiles/wofi/.config/wofi/assets/shuffle.png" -resize "80x80" \) \
...
if [ -n "$selected" ]; then
    ...
    if [ -n "$original_path" ]; then
        ...
        swww img --transition-type grow --transition-pos 0.854,0.977 --transition-step 90 "$original_path"
